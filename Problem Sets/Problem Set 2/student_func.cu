...
// we
// multiply each weight with the pixel underneath it. Finally, we add up all of
// the
...
// width.
// We refer to the array of weights as a filter, and we refer to its width with
// the
...
// for
// catching mistakes. If you write code the unsafe way and you make a mistake,
// then
// any subsequent kernels won't compute anything, and it will be hard to figure
// out
...
__global__ void gaussian_blur(const unsigned char* const inputChannel,
  ...
  float* s_filter = (float*)shared_buffer;
  unsigned char* s_inputChannel =
      (unsigned char*)&s_filter[filterWidth * filterWidth];
  // Initialize shared memory filter
  if (threadIdx.x < filterWidth && threadIdx.y < filterWidth) {
    s_filter[threadIdx.x + filterWidth * threadIdx.y] =
        filter[threadIdx.x + filterWidth * threadIdx.y];
  }
  // Initialize shared memory image chunk
  int width = BLOCK_SIZE + filterWidth - 1; // Width of 'extended' block

  int image_start_x = blockIdx.x * blockDim.x - (filterWidth - 1) / 2;
  int image_start_y = blockIdx.y * blockDim.y - (filterWidth - 1) / 2;

  int thread_index = threadIdx.y * blockDim.x + threadIdx.x;
  for (int i = 0; i < 4; i++) {
    int index_x = (thread_index * 4 + i) % width;
    int index_y = (thread_index * 4 + i) / width;
    if (index_x < width && index_y < width) {
      int x = image_start_x + index_x;
      x = (x < 0) ? 0 : x;
      x = (x < numCols) ? x : numCols - 1;

      int y = image_start_y + index_y;
      y = (y < 0) ? 0 : y;
      y = (y < numRows) ? y : numRows - 1;

      s_inputChannel[index_y * width + index_x] = inputChannel[y * numCols + x];
    }
  }
  ...
  if (x < numCols && y < numRows) {
    ...
    for (int filter_y = 0; filter_y < filterWidth; filter_y++) {
      for (int filter_x = 0; filter_x < filterWidth; filter_x++) {
        // Making sure the filter is applies in the image boundaries
        int i = threadIdx.x, j = threadIdx.y;
        i += filter_x;
        j += filter_y;
        int index = j * width + i;
        float image_value = s_inputChannel[index];
        float filter_value = s_filter[filter_y * filterWidth + filter_x];
        pixel += image_value * filter_value;
      }
      outputChannel[y * numCols + x] = pixel;
    }
  }
  // TODO
  ...
  // some of
  // its neighbors are outside the image, then you will need to be extra
  // careful. Instead
  // of trying to read such a neighbor value from GPU memory (which won't work
  // because
  // the value is out of bounds), you should explicitly clamp the neighbor
  // values you read
  // to be within the bounds of the image. If this is not clear to you, then
  // please refer
  // to sequential reference solution for the exact clamping semantics you
  // should follow.
}
...
void your_gaussian_blur(const uchar4* const h_inputImageRGBA,
  ...
  const dim3 gridSize(numCols / BLOCK_SIZE + 1, numRows / BLOCK_SIZE + 1, 1);

  // shared memory size
  const int s_mem_size = sizeof(float) * filterWidth * filterWidth + // filter
                         sizeof(unsigned char) *
                             (BLOCK_SIZE + filterWidth - 1) *
                             (BLOCK_SIZE + filterWidth - 1); // image region
  ...
  // channel.
  gaussian_blur << <gridSize, blockSize, s_mem_size>>>
